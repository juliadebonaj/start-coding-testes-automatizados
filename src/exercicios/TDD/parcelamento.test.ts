...
describe('calcularParcelamento', () => {
  describe('sem juros (1x a 4x)', () => {
    it('retorna o valor total em parcela única quando for 1x', () => {
      ...
      expect(resultado).toMatchObject({ valorParcela: 1000, totalParcelas: 1, valorTotal: 1000 })
    })

    it('divide o valor sem juros quando for 4x', () => {
      const resultado = calcularParcelamento(1000, 4)

      expect(resultado).toMatchObject({ valorParcela: 250, totalParcelas: 4, valorTotal: 1000 })
    })
  })
  ...
  describe('com juros', () => {
    it('aplica 5% sobre o total quando for de 5x a 8x', () => {
      // 1000 + 5% = 1050
      expect(calcularParcelamento(1000, 5)).toMatchObject({ valorParcela: 210, totalParcelas: 5, valorTotal: 1050 })
      expect(calcularParcelamento(1000, 8)).toMatchObject({ valorParcela: 131.25, totalParcelas: 8, valorTotal: 1050 })
    })

    it('aplica 8% sobre o total quando for de 9x a 12x', () => {
      // 1000 + 8% = 1080
      expect(calcularParcelamento(1000, 9)).toMatchObject({ valorParcela: 120, totalParcelas: 9, valorTotal: 1080 })
      expect(calcularParcelamento(1000, 12)).toMatchObject({ valorParcela: 90, totalParcelas: 12, valorTotal: 1080 })
    })

    it('aplica 10% sobre o total quando for de 13x a 18x', () => {
      // 1000 + 10% = 1100
      expect(calcularParcelamento(1000, 13)).toMatchObject({ valorParcela: 84.62, totalParcelas: 13, valorTotal: 1100 })
      expect(calcularParcelamento(1000, 18)).toMatchObject({ valorParcela: 61.11, totalParcelas: 18, valorTotal: 1100 })
    })
  ...
  describe('arredondamento', () => {
    it('arredonda o valor da parcela para 2 casas decimais', () => {
      // 100 / 3 = 33.333... -> 33.33
      expect(calcularParcelamento(100, 3)).toMatchObject({ valorParcela: 33.33, totalParcelas: 3, valorTotal: 100 })
    })
  })
